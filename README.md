...
flowchart LR
    ...
    subgraph pipeline["🧠 MaNA v4 叙事管线"]
        direction TB
        
        subgraph L0["L0: Context"]
            CB["ContextBuilder<br/>场景上下文组装"]
        end

        subgraph L1["L1: Director"]
            SD["SceneDirector<br/>节拍规划"]
            B3["Best-of-3 Scorer"]
            MV["Multi-View Synthesizer"]
        end

        subgraph L2["L2: Character Engines (N 路并行)"]
            ME["MotivationEngine"]
            DW["DialogueWeaver"]
            AD["ActionDirector"]
        end
        ...
        subgraph L3["L3: Composition"]
            SC["SceneComposer<br/>叙事编织"]
            RF["Refinement Loop<br/>Composer⇄Auditor"]
        end

        subgraph L3b4a["L3b ∥ L4a (并行)"]
        ...
        subgraph L4b["L4b"]
            TM["ThreadManager<br/>线索生命周期"]
        end

        subgraph L5["L5 (条件触发)"]
            RO["ReflectionOracle<br/>每 5 beats"]
        end
    end

    subgraph storage["💾 存储"]
        FS["FileStorage"]
        SAVE["SaveManager<br/>3 槽位"]
    end
    ...
    style pipeline fill:#0f3460,stroke:#16213e,color:#e0e0e0
