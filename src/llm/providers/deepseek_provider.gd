...
## 异步发起聊天请求
## OpenAI 兼容格式: {"model": "...", "messages": [...], "temperature": 0.7, "max_tokens": 1024}
func chat_async(system_prompt: String, user_message: String, options: Dictionary = {}) -> Dictionary:
	...
	var endpoint: String = _config.get("endpoint", "https://api.deepseek.com/v1/chat/completions")
	...
	for attempt in range(max_retries + 1):
		...
		if attempt < max_retries:
			push_warning("DeepSeek retry %d/%d: %s" % [attempt + 1, max_retries, result.get("error", "")])
...
func _build_request_body(system_prompt: String, user_message: String, options: Dictionary) -> Dictionary:
	...
	var body: Dictionary = {
		"model": options.get("model", _config.get("model_strong", "deepseek-chat")),
		"messages": messages,
...
## 解析 OpenAI 兼容格式的响应
## 格式: {"choices": [{"message": {"content": "...", "role": "assistant"}}], "usage": {"total_tokens": N}}
