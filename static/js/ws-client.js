...
export class WSClient {
  ...
  _routeMessage(msg) {
    ...
    // ── canon_generation_status → 透传 ──
    if (type === 'canon_generation_status') {
      // 直接由 app.js 的 handleGenerationStatus 处理
    }

    // ── 偏离度更新 ──
    if (type === 'deviation_update') {
  ...
  _applyStateSync(payload) {
    if (payload.beat_count !== undefined) {
      ...
      document.getElementById('beatCount').textContent = payload.beat_count;
