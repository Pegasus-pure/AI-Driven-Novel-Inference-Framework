...
from .memory.persist import MemoryPersister
...
class MemoryManager:
    ...
    def __init__(self, config: Optional[dict] = None, persist_dir: str = "") -> None:
        self.config: dict = config or {}

        # 持久化
        persist_enabled = bool(self.config.get("persist", False))
        self._persister: Optional[MemoryPersister] = (
            MemoryPersister(persist_dir) if persist_enabled and persist_dir else None
        )
    ...
    def add_memory(self, entry: MemoryEntry) -> None:
        ...
        # 持久化：实时追加到磁盘
        if self._persister and self._current_title:
            self._persister.append(self._current_title, entry.agent_id, entry.to_dict())
    ...
    def load_from_disk(self, title: str) -> None:
        """从磁盘加载全部记忆到内存流"""
        if not self._persister:
            return
        self._current_title = title
        self._persister.set_title(title)
        all_entries = self._persister.load_all(title)
        for agent_id, raw_entries in all_entries.items():
            for raw in raw_entries:
                entry = MemoryEntry.from_dict(raw)
                self.add_memory(entry)
        _log.info("从磁盘加载记忆: %d agents, 标题=%s", len(all_entries), title)

    def flush_to_disk(self, title: str) -> None:
        """将当前内存流持久化到磁盘（追加模式，只写新增条目）"""
        if not self._persister:
            return
        self._current_title = title
        self._persister.set_title(title)
        # 只 flush 最近的一批（用时间戳比较：上次 flush 后的新条目）
        # 简单实现：遍历所有 agent，写入所有条目（幂等？不，JSONL 会重复）
        # 更好的：在 add_memory 时也追加到磁盘
        _log.debug("flush_to_disk: title=%s memory_stream keys=%s", title, list(self.memory_stream.keys()))
