...
Internal storage uses a flat section-key dict (self._cfg_dict) instead of ConfigParser.
Legacy todo: ConfigParser removed 2026-06-22.

Provides access to:
  - Three-tier provider configs (strong / medium / light)
  - Retry / Oracle interval settings
  - v4 feature flags (refinement, director_best_of_n, multi_view, etc.)
...
# Tier name mapping: 所有变体 → 内部标识 strong/medium/light
# 内部代码统一使用 strong/medium/light
# config.yaml 原用中文层名，已统一为英文；保留旧名兼容旧存档
TIER_MAP: dict[str, str] = {
    "strong": "strong",
    "medium": "medium",
    "light": "light",
    # 向后兼容 config.yaml 旧版本（中文层名）
    "导演层": "strong",
    "演员层": "medium",
    "动作层": "light",
...
# 反向映射
_INVERSE_TIER_MAP: dict[str, str] = {
    "strong": "strong",
    "medium": "medium",
    "light": "light",
    "导演层": "strong",
...
class MananaConfig:
    ...
    Internal storage: self._cfg_dict is a dict of {section: {key: str_value, ...}}
    where all values are stored as strings, matching the old ConfigParser contract.
    """
    ...
    def __init__(self, yaml_dict: dict = None) -> None:
        ...
        """
        self._cfg_dict: dict[str, dict[str, str]] = {}
        self._loaded: bool = False
        self._yaml_dict: dict = yaml_dict or {}
    ...
    def _populate_from_yaml(self, cfg: dict) -> None:
        """从 config.yaml 的字典直接构建内部配置 dict。"""
        cfg = cfg or {}
        ...
        # 全局
        self._cfg_dict["global"] = {"config_version": "2"}

        # 三级 Provider
        # config.yaml 使用中文层名（导演层/演员层/动作层），映射为 provider_strong 等 section
        # Tier 配置自包含，provider 模板已移除（template 条目不再存在）
        ...
        for internal_tier in _INTERNAL_TIERS:
            ...
            if not endpoint:
                endpoint = ""  # 无端点不自动回退，由 Provider 检查
            elif prov_type == "ollama":
                # 防止重复拼接：先清理已有 /api/chat 后缀
                endpoint = endpoint.rstrip("/").replace("/api/chat", "") + "/api/chat"
            else:
                endpoint = endpoint.rstrip("/") + "/v1/chat/completions"
            self._cfg_dict[section] = {
                "type": prov_type,
                "endpoint": endpoint,
                "api_key": str(t.get("api_key", "")),
                "model": str(t.get("model", "")),
                "temperature": str(t.get("temperature", 0.7)),
                "max_tokens": str(t.get("max_tokens", 2048)),
                "timeout": str(t.get("timeout", 120)),
            }

        # 功能开关
        self._cfg_dict["v4"] = {"enabled": "true"}
        for feat in ("multi_view", "dynamic_tier", "micro_oracle", "semantic_selection",
                      "emergence_system", "continuity_check", "role_reflection",
                      "memory_system", "prompt_optimization"):
            self._cfg_dict[feat] = {"enabled": "true" if features.get(feat, False) else "false"}

        # refinement（含子键）
        self._cfg_dict["refinement"] = {
            "enabled": "true" if features.get("refinement", True) else "false",
            "max_warning_refine": "1",
            "max_fail_rewrite": "2",
        }

        # multi_view (standalone feature flag, not in features list)
        multi_view_enabled = features.get("multi_view", False)
        self._cfg_dict["multi_view"] = {
            "enabled": "true" if multi_view_enabled else "false",
        }

        # director.best_of_n（导演层 Best-of-N）
        director = cfg.get("director", {}) or {}
        director_bon = director.get("best_of_n", {}) or {}
        self._cfg_dict["director_best_of_n"] = {
            "enabled": "true" if director_bon.get("enabled", True) else "false",
            "sample_count": str(director_bon.get("sample_count", 3)),
            "temperatures": director_bon.get("temperatures", [0.4, 0.6, 0.8]),
        }

        # retry / oracle
        self._cfg_dict["retry"] = {
            "max_retries": "3",
            "base_delay": "1.0",
        }
        self._cfg_dict["oracle"] = {
            "trigger_interval": str(game.get("oracle_interval", 5)),
        }

        # emergence / continuity / reflection
        emerge = cfg.get("emergence", {}) or {}
        self._cfg_dict["emergence"] = {
            "hit_threshold": str(emerge.get("hit_threshold", 3)),
            "similarity_threshold": str(emerge.get("similarity_threshold", 0.75)),
            "feature_extraction": str(emerge.get("feature_extraction", "llm")),
            "max_pending_entities": str(emerge.get("max_pending_entities", 50)),
        }

        cont = cfg.get("continuity", {}) or {}
        self._cfg_dict["continuity"] = {
            "max_rewrite": str(cont.get("max_rewrite", 2)),
            "tier": resolve_tier(str(cont.get("tier", "medium"))),
        }

        refl = cfg.get("reflection", {}) or {}
        self._cfg_dict["reflection"] = {
            "tier": resolve_tier(str(refl.get("tier", "light"))),
        }
        for key in ("check_clothing", "check_location", "check_mood", "check_relationship"):
            self._cfg_dict["reflection"][key] = "true" if refl.get(key, True) else "false"

        # truncation 配置节
        trunc = cfg.get("truncation", {}) or {}
        self._cfg_dict["truncation"] = {
            "thread_context": str(trunc.get("thread_context", 3000)),
            "llm_extract": str(trunc.get("llm_extract", 15000)),
            "scene_context": str(trunc.get("scene_context", 4000)),
            "narrative_history": str(trunc.get("narrative_history", 2000)),
        }

        # memory system
        mem = cfg.get("memory", {}) or {}
        self._cfg_dict["memory"] = {
            "recency_weight": str(mem.get("recency_weight", 0.4)),
            "relevance_weight": str(mem.get("relevance_weight", 0.3)),
            "importance_weight": str(mem.get("importance_weight", 0.3)),
            "decay_lambda": str(mem.get("decay_lambda", 0.05)),
            "reflection_threshold": str(mem.get("reflection_threshold", 30)),
            "top_k_director": str(mem.get("top_k_director", 5)),
            "top_k_character": str(mem.get("top_k_character", 3)),
            "max_entries_per_agent": str(mem.get("max_entries_per_agent", 200)),
            "retrieve_recency_window": str(mem.get("retrieve_recency_window", 100)),
            "retention_window": str(mem.get("retention_window", 50)),
            "low_importance_threshold": str(mem.get("low_importance_threshold", 4.0)),
            "compact_interval": str(mem.get("compact_interval", 10)),
        }

    def reload(self, yaml_dict: dict) -> None:
        """从新的 YAML dict 重新加载配置（热重连用）"""
        self._yaml_dict = yaml_dict or {}
        self._cfg_dict = {}
        self._populate_from_yaml(yaml_dict)
        self._loaded = True
    ...
    def set_tier_config(self, tier: str, config: dict) -> None:
        ...
        if section not in self._cfg_dict:
            self._cfg_dict[section] = {}
        self._cfg_dict[section].update({
            "type": str(config.get("type", "ollama")),
            "endpoint": str(config.get("endpoint", "")),
            "api_key": str(config.get("api_key", "")),
            "model": str(config.get("model", "")),
            "temperature": str(config.get("temperature", 0.7)),
            "max_tokens": str(config.get("max_tokens", 2048)),
    ...
    def is_feature_enabled(self, feature: str) -> bool:
        ...
        Args:
            feature: Section name such as "refinement", "director_best_of_n", "multi_view", etc.
    ...
    def get_director_best_of_n_config(self) -> dict:
        """Get Best-of-N Director parameters."""
        return {
            "sample_count": self._get_int("director_best_of_n", "sample_count", 3),
            "temperatures": self._cfg_dict.get("director_best_of_n", {}).get("temperatures", [0.4, 0.6, 0.8]),
        }
    ...
    def _get_bool(self, section: str, key: str, default: bool = False) -> bool:
        section_dict = self._cfg_dict.get(section)
        if section_dict is None:
            return default
        val = section_dict.get(key, str(default)).lower()
    ...
    def _get_float(self, section: str, key: str, default: float = 0.0) -> float:
        ...
        if section_dict is None:
            return default
        try:
            return float(section_dict.get(key, str(default)))
    ...
    def _get_int(self, section: str, key: str, default: int = 0) -> int:
        ...
        if section_dict is None:
            return default
        try:
            return int(section_dict.get(key, str(default)))
        except (ValueError, TypeError):
            return default
