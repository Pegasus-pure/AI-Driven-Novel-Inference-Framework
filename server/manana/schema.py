...
class MananaSchema:
    ...
    def validate_composer_output(data: dict) -> dict:
        ...
        result = MananaSchema._validate_keys(data, MananaSchema.COMPOSER_OUTPUT_KEYS, MananaSchema._COMPOSER_TYPE_MAP)
        # Also validate choices if present
        choices = data.get("choices")
        if choices is not None:
            choices_result = MananaSchema.validate_choices(choices)
            if not choices_result.get("valid", False):
                errors = result.get("errors", [])
                errors.extend(choices_result.get("errors", []))
                result["valid"] = False
                result["errors"] = errors
        return result

    @staticmethod
    def validate_choices(choices: list) -> dict:
        """Validate a list of choice objects.

        Each choice must contain id, text, hint, and next_scene_hint fields.

        Args:
            choices: List of choice dicts to validate.
        ...
        if not isinstance(choices, list):
            return {"valid": False, "errors": ["choices must be a list"]}

        for i, choice in enumerate(choices):
            if not isinstance(choice, dict):
                errors.append(f"choices[{i}] expected dict, got {type(choice).__name__}")
                continue
            for field in ("id", "text", "hint", "next_scene_hint"):
                if field not in choice:
                    errors.append(f"choices[{i}] missing required field: '{field}'")
                elif not isinstance(choice[field], str):
                    errors.append(f"choices[{i}].{field} expected str, got {type(choice[field]).__name__}")
            # Validate id format: c1, c2, c3...
            cid = choice.get("id", "")
            if cid and not isinstance(cid, str):
                errors.append(f"choices[{i}].id must be a string")
            ...
            for i, ch in enumerate(choices):
                if isinstance(ch, dict):
                    for field in ("text", "hint"):
                        if not str(ch.get(field, "") or "").strip():
                            errors.append(f"choices[{i}].{field} 为空")
                    # 检查 next_scene_hint 不为空
                    if not str(ch.get("next_scene_hint", "") or "").strip():
                        errors.append(f"choices[{i}].next_scene_hint 为空")
