...
# 导入存储后端
from server.storage import get_storage, CanonStorage
from server.storage.file_storage import FileStorage

_log = logging.getLogger("Rain.CanonManager")
...
class CanonManager:
    ...
    def create_running_canon(
        ...
        Args:
            ...
            initial_data: 可选，已读取的完整 Canon 数据。传入后跳过形态B→A回读。
                         形态B只用于后续增/改/删条目，_running_canon 保持完整。
        ...
        if ok:
            ...
            if initial_data is not None:
                # ★ 直接用完整数据，不经过形态B→A回读（避免丢失timeline/writing_style等字段）
                self._running_canon = dict(initial_data)
                self._running_canon["_source"] = "running"
                self._running_canon["_initial_source"] = initial_canon_path
            else:
                # 回退：走旧的形态B→A重建（少部分字段会丢，但不会崩）
    ...
    def create_empty_running_canon(self, title: str) -> bool:
        """创建空白运行 Canon（手动从头构建世界）

        不依赖任何初始 Canon JSON 文件。创建后玩家通过 F2/F3/F4 面板手动填入角色/地点/世界观。
        ...
        """
        if self.is_running():
            _log.warning("运行 Canon 已存在，跳过创建")
            return False
        ...
        empty_canon: dict = {
            ...
            "timeline": [],
            "meta": {"author": "", "synopsis": "", "tags": []},
            "_source": "manual",
        }

        self._running_canon = empty_canon
        self._current_novel = title
        self._initial_source = "manual"

        # 写入存储（创建 novel/{title}/ 目录结构）
        self.save_running_canon()
        _log.info("空白运行 Canon 已创建: %s", title)
        return True
    ...
    def get_running_canon(self) -> Optional[dict]:
        """获取当前运行时 Canon（完整形态A数据，非目录结构重建）"""
        return self._running_canon
    ...
    def _generate_id(self, section: str) -> str:
        ...
        使用 uuid4 确保 ID 在多线程/多进程环境下也不会重复。

        Args:
            section: 分区 — 'characters' | 'locations'

        Returns:
            新 ID (如 "char_a1b2c3d4", "loc_e5f6g7h8")
        """
