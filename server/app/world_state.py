...
from server.manana.soul.soul_state import SoulPossessionState, NPCCognitiveDissonance

# 兼容 server/ 和 project_root 两种导入上下文
try:
    from server.manana.contextual_memory import MemoryManager as _MemoryManager
except ImportError:
    from manana.contextual_memory import MemoryManager as _MemoryManager
...
class WorldState:
    ...
    def __init__(self) -> None:
        ...
        # ── 灵魂附生（soul_possession 模式） ──
        self.game_mode: str = "soul_possession"
        """游戏模式: soul_possession（唯一模式）"""
        self.soul_possession: Optional["SoulPossessionState"] = None
        """灵魂附生状态"""
        self.cognitive_dissonance: dict[str, Any] = {}
        """NPC 认知冲突映射 {char_id: NPCCognitiveDissonance}"""

        # ── 主角 ID（灵魂附生模式下，player == 该角色） ──
        self._protagonist_id: str = ""
    ...
    def _get_protagonist_state(self) -> dict:
        """获取主角在 characters_state 中的条目（不存在返回空 dict）"""
        if not self._protagonist_id:
            return {}
        return self.characters_state.get(self._protagonist_id, {})

    def _ensure_protagonist_in_characters(self) -> None:
        """确保主角已注册到 characters_state（仅 world-facing 字段：name/location/mood）"""
        if not self._protagonist_id:
            return
        if self._protagonist_id not in self.characters_state:
            cs = {
                "name": self.player_profile.get("name", self._protagonist_id),
                "mood": "中性",
                "attitude": "中立",
                "location": self.player_location,
            }
            self.characters_state[self._protagonist_id] = cs

    def _sync_player_location_to_protagonist(self) -> None:
        """将 player_location 同步到 protagonist 的 characters_state.location"""
        if not self._protagonist_id or not self.player_location:
            return
        if self._protagonist_id in self.characters_state:
            self.characters_state[self._protagonist_id]["location"] = self.player_location

    def _sync_protagonist_location_to_player(self) -> None:
        """将 protagonist 的 characters_state.location 同步到 player_location"""
        if not self._protagonist_id:
            return
        loc = self._get_protagonist_state().get("location", "")
        if loc:
            self.player_location = loc

    def reconcile_player_state(self) -> None:
        """调平 player ↔ protagonist 的共享状态（仅位置，人格保持分离）"""
        if not self._protagonist_id:
            return
        self._ensure_protagonist_in_characters()
        self._sync_player_location_to_protagonist()
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "soul_possession": self.soul_possession.to_dict() if hasattr(self.soul_possession, 'to_dict') else {},
            "_protagonist_id": self._protagonist_id,
            "cognitive_dissonance": {
                k: v.to_dict() if hasattr(v, 'to_dict') else v
                for k, v in self.cognitive_dissonance.items()
            },
        }
    ...
    def from_dict(cls, data: dict) -> "WorldState":
        ...
        # 恢复灵魂附生（唯一模式，始终恢复 soul_possession 数据）
        ws.game_mode = "soul_possession"
        soul_data = data.get("soul_possession", None)
        if soul_data:
            ws.soul_possession = SoulPossessionState.from_dict(soul_data)
            dissonance_raw = data.get("cognitive_dissonance", {})
            ws.cognitive_dissonance = {
                k: NPCCognitiveDissonance.from_dict(v)
                for k, v in dissonance_raw.items()
            }
        # 恢复主角 ID
        ws._protagonist_id = str(data.get("_protagonist_id", ""))
    ...
    def apply_patch(self, patch: dict) -> None:
        ...
        # ★ 双向同步 player ↔ protagonist
        self.reconcile_player_state()
    ...
    def apply_mood_change(self, char_id: str, new_mood: str, intensity: Optional[float] = None) -> None:
        """更新角色情绪状态"""
    ...
    def apply_location_change(self, char_id: str, to_loc: str) -> None:
        ...
        if char_id and to_loc:
            cs = self.characters_state.setdefault(char_id, {})
            cs["location"] = to_loc
            # ★ 如果是主角，同步 player_location
            if char_id == self._protagonist_id:
                self.player_location = to_loc
        ...
        for nt in (updates.get("new_threads", []) or []):
            ...
            if title:
                new_t = {
                    ...
                    "involved_characters": list(nt.get("involved_characters", []) or []),
