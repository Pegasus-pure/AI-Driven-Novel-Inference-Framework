...
_log = logging.getLogger("AINovelFramework.GameSession")
...
class GameSession:
    ...
    async def initialize(self, config_path: str = "") -> None:
        ...
        # 加超时保护，避免阻塞 WS 连接
        try:
            from .manana.pipeline import MananaPipeline
    ...
    def _sync_characters_state_from_canon(self, canon: dict) -> None:
        ...
        for c in canon.get("characters", []) or []:
            ...
            cs.setdefault("is_present", False)
            # 更新死亡状态
            if status == "dead":
                cs["status"] = "dead"
    ...
    async def start_llm_generation_with_progress(
        ...
        async def _generation_worker() -> None:
            ...
            try:
                ...
                # 启动周期性进度推送
                progress_task = asyncio.ensure_future(_periodic_progress())
        ...
        # 启动异步生成任务（不阻塞当前消息循环）
        self._generation_task = asyncio.ensure_future(_generation_worker())
    ...
    async def run_beat(self, player_action: str, progress_cb=None) -> dict:
        ...
        try:
            ...
            _log.error("Pipeline run_beat 异常: %s", exc)
    ...
    def get_config_info(self) -> dict:
        ...
        # 将中文层名映射回内部名（用相对导入，兼容 sys.path 环境）
        try:
            from .manana.config import resolve_tier
        except ImportError:
    ...
    async def update_config(
        ...
        import shutil
        from pathlib import Path
        from .manana.config import resolve_tier, TIER_MAP
    ...
    def _get_present_character_ids(self) -> list[str]:
        ...
        for cid, cs in self.world_state.characters_state.items():
            ...
            if not player_loc:
                # 玩家位置未知 → 只显示标记为 is_present 的角色
                if cs.get("is_present", False):
                    present.append(cid)
